...
function ExistShareItemModal(props){
    ...
    const API_URL_ExistShareItem   = store.getState().config.API_URL_ExistShareItem;
    ...
    const API_URL_UpdatePermission = store.getState().config.API_URL_UpdatePermission;
  ...
  async function ExistShareItem(event){
    ...
    if(userFound){
      axios.post(API_URL + API_URL_ExistShareItem, { 
        ...
        location:store.getState().startLocation,
